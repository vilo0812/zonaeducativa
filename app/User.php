...
class User extends Authenticatable implements JWTSubject
{
    ...
    public function getJWTIdentifier()
      ...
      public function scopeIdentificationCard($query,$ic){
        $data = $query->where('identification_card','LIKE',"%$ic%")
        ->whereIn('users.rol_id',[2,4]);
          return $data;
      }
      ///end scope del ID
      ...
      public function scopePhone($query,$phone){
        $data = $query->where('phone','LIKE',"%$phone%")
          ->whereIn('users.rol_id',[2,4]);
          return $data;
      }
      ...
      public function scopeEmail($query,$email){
        $data = $query->where('email','LIKE',"%$email%")
        ->whereIn('users.rol_id',[2,4]);
          return $data;
      }
      ///end scope del telefono
